...
2. An Easy search on any topic of interest and get deeper information on each
...
6. Most important - Stories that these data tell, e.g., Evolution of the American Worker, Poverty is bad for your health, Men still dominate in the highest paying industries, Opioid addiction damage and so many others. 
...
## Creating A Diverging Bars Chart

A diverging bars chart shows and compares positive and negative values for a particular variable.  One popular use case is survey analysis, in which multiple options are given as the categories in the bar chart, and there are two opposite ends of the spectrum for values.  These two sides are usually 'positive' vs. 'negative', but they can also be categorical values such as 'true' or 'false'.  

Because these graphs can be more complicated to explain than to understand visually, below is a diverging bars chart created in R.  Reference for the code: [@viz_R]

```{r}
data("mtcars")  #use data from R base
...
ggplot(mtcars, aes(x=`car name`, y=mpg_z, label=mpg_z)) + 
  ...
  coord_flip()
```
In addition to R, Tableau is another convenient tool for creating these diverging bar charts.  Below is an example of such a chart created using survey responses data:
