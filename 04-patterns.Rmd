...
```
4.1. Regular quaterly sales. We can see sales decreased a lot around 2014.  **The baseline here is historical sales.**
```{r}
...
4.2. Quaterly and yearly change sales. **The baseline here is zero and look at the percentage changes.**
```{r}
 # Quaterly change
...
4.3. The sales difference compare to now. **The baseline here is the current sales.**
```{r}
...
```
4.4. Sales difference compared to the first quater. ** The baseline here is the first quater sales.**
```{r}
...
4.5. The difference between quater sales and mean. ** The baseline is mean now.**
```{r}
...
## 5. Using design patterns to find greater meaning in your data
...
## 6. Example Visualizations of Time Series Data
...
## 8 Tips to improve Data Visualization
**8.1.Comparison**
...
** 8.6.Use layers to tell a story**
...
## 9. More ways to improve your visualization design
...
## 11. Reusable Data Visualization Code in R
[@viz_R]
...
To provide an example the "layering" mentioned above, here is a generic snippet of code for creating a scatterplot with ggplot2 and the mtcars dataset in R base, using this website's code as a template:
```{r}
library(ggplot2)
