...
In order to understand the type of visualization project that you are dealing with, you must first understand the purpose of your project: Are you trying to simply understand the data, or are you trying to convey a message and provoke an action?

According to Khai Nguyen, 

1. Understanding is 

>often a collection of basic charts and graphs. The goal is to present the data in an in-depth and unbiased way.

An example of a visualization for understanding is
![understanding](images/understanding.png)

The understanding is more like 'to inform', where the main goal is to deliver knowledge effectively to the audience. The situations can be a board meeting, a earnings call, a monthly sales report and etc. Data vizs are used to present facts here. It is more appropriate to display a single number or simple and basic charts for easier understanding, instead of complex charts with advanced features. Little background of the data presented is needed here.


2. Persuasion is 

>less structured, with no obvious way to implement. The goal is to present the data in a way that can create powerful impacts on viewers.

An example of a visualization for persuasion is:
![persuasion](images/persuasion.png)

For the 'to persuade' type, the main goal is to prove a point to the audience and trigger attention, change or decision making. The situations can be a proposal of business decisions, a meeting to find out why sales drops or an advocate of energy saving actions. Data vizs are used to find reasons behind facts, or compare different situations. The charts are more complicated and much background information is needed to the data presented. It is very import to tell a story with logic, explaining from begining to end carefully.

This is similar to the Chicago mayor project we have done, where we are trying to give suggestions to the mayor to improve on the automated speed enforcement program.
![chicago](images/Speed camera violation.png)


After figuring out the purpose of your visualiation, continue to read how different sectors use visualizations differently.
...
Within the business sector, visualizations can be used very differently by different people; here's just a short list of common business users of visualizations:
...
And for these different end users, the type of visualizations used will vary dramatically as the complexity and informative level required are dissimilar.
