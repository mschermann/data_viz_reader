...
Ref:https://blog.hootsuite.com/how-to-visualize-social-media-data/

 In order to sell the value of social media, its important to present the data clearly in order to make a persuasive case. These article focuses on   important aspects of visualizing social media data.

1.    Know your graphs: All graphs aren't created equal-each one is designed to display data in a certain                way and some are better than others. Basic use of some of the charts are:

�     Scatter plot-To show correlation.

�     Pie chart-To show proportion. You should not  have more than five slivers. In case more than five is required, its better to use something other than a pie chart.

�     Line graph-Useful To show trends and pattern, stimeseries.

�     Bar chart-To show comparisons.

�     Table-To show precise values.

2.          Keep color in mind: While its tempting to use lot of colors to make charts attractive but it should be avoided. Too many colors distracts audience.Author has mentioned the following in regards to color:

�       Avoid Red: Even though we are trying to highlight something positive but whenever we see red in a visualization, we think that something went wrong.

�       Use color gradients to display a time series-Use shades from lightest to darkest can signify past to present.

�       Highlight-A strong hue can be used  to emphasize a particular value  and draw audience attention.

�       Give each color a value-To use color to represent a range. For example, blue for anything over 90 percent and orange for anything below 10 percent.

�       Use color to group relevant data together-For example blue for Twitter, yellow for Facebook, and pink for Instagram.

3.     Know your audience: To effectively engage audience, its very important to know them and adapt          the visualization accordingly

4.    Make it easy to understand: User experience is very important when delivering data insights. As    explained in the article few things to keep in mind are:

The mind works left to right-For  charts that are meant to compare values, the values should be presented in an ascending or descending order-so that audience does not have  to go back and forth between values to compare them.
Do the work for your audience-Sort data, group values together if possible, and highlight important points.
Don't be overly focused on aesthetics-Refrain from using a pie chart when simply writing out numbers will do.
Keep it simple- Stay away from anything that will clutter visualization and let the data speak for itself.
